...
macro_rules! test_file {
    ($name:ident, $path:literal) => {
        ...
        fn $name() {
            ...
            let sections = parse_sections(&file).expect("could not parse section");
            let _config = Config::try_from(&sections[..]).expect(&format!(
                "failed to parse config from sections: {sections:#?}"
            ));
        }
    };
}
